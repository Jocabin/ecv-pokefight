...
<h1 class="text-2xl font-bold text-center">Choisissez votre pokemon</h1>
...
<form method="post" class="p-4 mt-4">
	...
	<div class="fixed left-0 bottom-0 w-full bg-white p-4 border-t flex items-stretch justify-between gap-4">
		<input type="text" name="pokeName" value="{choosenPokemon.name}" class="border-black border rounded text-2xl p-2 flex-1">
		<button type="submit" class="bg-accent text-white font-bold uppercase text-3xl p-4 rounded">Créer un combatant</button>
	</div>
</form>
